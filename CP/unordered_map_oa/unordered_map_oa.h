...
namespace CP{
    ...
    class QuadraticProbing{
        public:
            size_t operator()(size_t initial_pos,size_t col_count, size_t bucket_count){
                return (initial_pos + col_count*col_count)%bucket_count;
            }
    };

    template<typename KeyT,typename MappedT,typename HasherT = std::hash<KeyT>,
             typename EqualT = std::equal_to<KeyT>,typename NextAddressT = LinearProbing>
    ...
    class unordered_map{
        protected:
            ...
            class hashtable_iterator{
                protected:
                    ...
                    void to_next_data(){
                        while(mCurBucketItr != mEndBucketItr && !mCurBucketItr->has_data()){
                            mCurBucketItr++;
                        }
                    }
                public:
        ...
        public:
            ...
            // default constructor
            unordered_map():mBuckets(std::vector<BucketT>(11)),mSize(0),mHasher(HasherT()),mEqual(EqualT()),mMaxLoadFactor(0.5),
            ...
            MappedT& operator[](const KeyT& key){
                ...
                if(mBuckets[pos].available()) BucketIterator it = insert_to_position(std::make_pair(key, MappedT()),pos);
