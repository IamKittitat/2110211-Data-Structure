...
bool test3() {
  CP::map_avl<TestClass,float> m;
  CP::map_avl<TestClass,float> m2;
...
bool test4() {
  ...
  CP::map_avl<TestClass,float,ComparatorClass> m;
  for (auto& x : data) {
    m[x.first] = x.second;
  }
  m.print();

  CP::map_avl<TestClass,float,CompFunc> m2(comparator);
  for (auto& x : data) {
    m2[x.first] = x.second;
  }
  m2.print();
