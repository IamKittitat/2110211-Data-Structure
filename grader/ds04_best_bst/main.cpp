...
void insert_new(int first,int last,CP::map_bst<int,int> &bst){
    int half = (first+last)/2;
    bst[half] = half;
    //std::cout << "FIRST : " << first << " LAST : " << last << " HALF : " << half << std::endl;
    if((half+last)/2==half && (half+first)/2==half) return;
    insert_new(first,half,bst);
    insert_new(half+1,last,bst);
}
